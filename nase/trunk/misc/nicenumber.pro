...
;
; PURPOSE:
;  Find a nice number close to the given number (1, 2, 2.5, or 5
;  scaled to the size of the given number). This routine can be
;  helpful to determine tick spacings and plot scaling. It is used by
;  <A>hist</A> and was written by R. Sterner at Johns Hopkins
...
;
; CALLING SEQUENCE:
;* n = NiceNumber(x [,/FLOOR][,/CEIL][,MINOR=...])
;
; INPUTS:
;  x:: Given number.
;
...
;
; OUTPUTS:
;  n:: Nice number close to <*>x</*>. 1, 2, 2.5, or 5 scaled to size
;      of <*>x</*> .
;
; OPTIONAL OUTPUTS:
;  MINOR:: Return suggested minor tick spacing.
;
; EXAMPLE:
...
;  <*>/FLOOR</*> and <*>/CEIL</*> may not give values related to the step size,
;  <*>dx</*>. The following method will:<BR> 
;  <*>ceil(t/dx)*dx</*> is a multiple of dx on the high side of t.<BR>
;  <*>floor(t/dx)*dx</*>  is a multiple of dx on the low side of t.<BR>
;
; SEE ALSO:
;  <A>hist</A>
;  
