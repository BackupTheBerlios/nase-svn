...
;
; OPTIONAL INPUTS:
;  n::  The total number of places (including sign) reserved for each string. (Default: The number of places needed.)
;
; INPUT KEYWORDS:
...
;
; EXAMPLE:
;* Help, IntStr(3.7), IntStr(3.7, 6), IntStr(4, 6, SPACE = '0'), IntStr(3, 6, SPACE = '')
;
;  IDL prints:
;
;* ><Expression>    STRING    = '4'
...
FUNCTION  IntStr,   X_, N_,   space = space_
   ...
   X = Long64(Round(X_))
   IF  TypeN NE 0  THEN  N = Round(N_(0))      ; If N is an array, only the first value is taken seriously.
