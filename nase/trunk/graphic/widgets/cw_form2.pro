...
;
;       CW_FORM2 is derived from the standard IDL-Routine CW_FORM, but the bug
;       occuring in the latter has been fixed. It is now possible to exit the 
;       modal form widget by pressing the window's CLOSE-button.
;
; CATEGORY:
;	GRAPHICS /WIDGETS
;
; CALLING SEQUENCE:
;	widget = CW_FORM2([Parent,] Desc)
;
; INPUTS:
...
;		and its value returned as the result of CW_FORM2.  For non-
...
;  *****************
;
;    To use CW_FORM2 inside another widget:
;	    a = widget_base(title='Testing')
;	    b = cw_form2(a, desc, /COLUMN)
...
function CW_FORM2_PARSE, Extra, Name, Value, Index=Index
...
pro CW_FORM2_APPEND, extra, e, keyword, USE_VALUE=use_value, ACTUAL_KEYWORD=akw
if CW_FORM2_PARSE(e, keyword, value) then begin
...
pro CW_FORM2_LABEL, parent, nparent, e, frame
;Put LABEL_LEFT and/or LABEL_RIGHT on a base.

nparent = parent
if CW_FORM2_PARSE(e, 'LABEL_LEFT', value) then begin
	nparent = WIDGET_BASE(nparent, /ROW, FRAME=frame)
	frame = 0
	junk1 = WIDGET_LABEL(nparent, VALUE=value)
	endif
if CW_FORM2_PARSE(e, 'LABEL_TOP', value) then begin
...
pro CW_FORM2_BUILD, parent, desc, cur, ids, lasttag
...
;
  ...
  while cur lt n do begin
	...
	quit = CW_FORM2_PARSE(e, 'QUIT')
	frame = CW_FORM2_PARSE(e, 'FRAME')
	efn = CW_FORM2_PARSE(e, 'EVENT', event_fun)
	CW_FORM2_APPEND, extra, e, 'FONT', /USE_VALUE
	CW_FORM2_APPEND, extra, e, 'COLUMN'
	CW_FORM2_APPEND, extra, e, 'ROW'
	CW_FORM2_APPEND, extra, e, 'LEFT', ACTUAL='ALIGN_LEFT'
	CW_FORM2_APPEND, extra, e, 'CENTER', ACTUAL='ALIGN_CENTER'
	CW_FORM2_APPEND, extra, e, 'RIGHT', ACTUAL='ALIGN_RIGHT'
    endif else e = ''
...
'BASE': BEGIN
    ...
    ENDCASE
'BUTTON': BEGIN
    CW_FORM2_APPEND, extra, e, 'LABEL_LEFT', /USE_VALUE
    CW_FORM2_APPEND, extra, e, 'LABEL_TOP', /USE_VALUE
    CW_FORM2_APPEND, extra, e, 'SET_VALUE', /USE_VALUE
    exclusive = CW_FORM2_PARSE(e,'EXCLUSIVE')
    no_release = CW_FORM2_PARSE(e,'NO_RELEASE')
...
process_integer:
    uextra = { VALUE: value }    
    CW_FORM2_LABEL, parent, nparent, e, frame
    if CW_FORM2_PARSE(e, 'WIDTH', temp) then width = fix(temp) else width=6
...
'LIST': BEGIN
    CW_FORM2_LABEL, parent, nparent, e, frame
    v = str_sep(a[2], '|')
    if CW_FORM2_PARSE(e, 'HEIGHT', temp) eq 0 then temp = n_elements(v)
...
ENDCASE
	...
	if CW_FORM2_PARSE(e, 'TAG', value) then value = STRUPCASE(value) $
    ...
    if dflags and 1 then CW_FORM2_BUILD, new, desc, cur, ids, lasttag  ;Begin new
...
Function CW_FORM2_EVENT, ev		;Event handler for CW_FORM2
...
Pro CW_FORM2_SETV, id, value	;In this case, value = { Tagname : value, ... }
...
Function CW_FORM2_GETV, id	;Return value of a CW_FORM2 widget.
...
pro cw_form2_modal_event, ev
   IF TAG_NAMES(ev, /STRUCTURE_NAME) EQ 'WIDGET_KILL_REQUEST' THEN BEGIN
      ...
      WIDGET_CONTROL, grandchild, GET_UVALUE=u ;Get handle
      WIDGET_CONTROL, child, GET_VALUE=v ;The widget's value
...
;  ON_ERROR, 2						;return to caller
  ...
  if n_params() eq 1 then begin
	...
	p = WIDGET_BASE(TITLE=title, Column = column, row=row, $
                        GROUP_LEADER=temp, /MODAL, /TLB_KILL_REQUEST_EVENTS)
  ...
  CW_FORM2_BUILD, base, desc, 0, ids, lasttag
  widget_control, base, EVENT_FUNC='CW_FORM2_EVENT', $
	FUNC_GET_VALUE='CW_FORM2_GETV', PRO_SET_VALUE='CW_FORM2_SETV'

  if n_params() eq 1 then begin		;Modal?
	WIDGET_CONTROL, p, /realize
	XMANAGER, 'CW_FORM2', p, EVENT_HANDLER='CW_FORM2_MODAL_EVENT'
        v = TEMPORARY(*handle)
	PTR_FREE, handle
