...
#define IDL_EXCLUDE_UNDEF(v) { if (!v->type) \
	IDL_MessageVarError(IDL_M_UNDEFVAR, v, IDL_MSG_LONGJMP); }
#define IDL_EXCLUDE_CONST(v)        { if (v->flags & IDL_V_CONST) \
	IDL_MessageVarError(IDL_M_NOCONST, v, IDL_MSG_LONGJMP); }
...
#define IDL_EXCLUDE_STRUCT(v)       { if (v->flags & IDL_V_STRUCT) \
	IDL_MessageVarError(IDL_M_NOSTRUCT, v, IDL_MSG_LONGJMP); }
...
#define IDL_EXCLUDE_COMPLEX(v)      { if ((v->type == IDL_TYP_COMPLEX) \
	...
	IDL_MessageVarError(IDL_M_NOCOMPLEX, v, IDL_MSG_LONGJMP); }
#define IDL_EXCLUDE_STRING(v)       { if (v->type == IDL_TYP_STRING)  \
	IDL_MessageVarError(IDL_M_NOSTRING, v, IDL_MSG_LONGJMP); }
#define IDL_EXCLUDE_SCALAR(v) { if (!(v->flags & IDL_V_NOT_SCALAR)) \
	IDL_MessageVarError(M_NOSCALAR, v, IDL_MSG_LONGJMP);}


/**** Ensure that variables possess certain attributes ****/
#define IDL_ENSURE_ARRAY(v) { if (!(v->flags & IDL_V_ARR)) \
	IDL_MessageVarError(IDL_M_NOTARRAY, v, IDL_MSG_LONGJMP); }
#define IDL_ENSURE_SCALAR(v) { if (v->flags & IDL_V_NOT_SCALAR) \
	IDL_MessageVarError(IDL_M_NOTSCALAR, v, IDL_MSG_LONGJMP);}
#define IDL_ENSURE_STRING(v) { if (v->type != IDL_TYP_STRING)  \
	IDL_MessageVarError(IDL_M_REQSTR, v, IDL_MSG_LONGJMP);}
...
#define IDL_ENSURE_STRUCTURE(v) { if (!(v->flags & IDL_V_STRUCT)) \
	IDL_MessageVarError(IDL_M_STRUC_REQ, v, IDL_MSG_LONGJMP);}
#define IDL_ENSURE_PTR(v) { if (v->type != IDL_TYP_PTR)  \
	IDL_MessageVarError(IDL_M_REQPTR, v, IDL_MSG_LONGJMP);}
#define IDL_ENSURE_OBJREF(v) { if (v->type != IDL_TYP_OBJREF)  \
	IDL_MessageVarError(IDL_M_REQOBJREF, v, IDL_MSG_LONGJMP);}
...
extern char *IDL_FilePathFromRoot IDL_ARG_PROTO((char *pathbuf, IDL_STRING
        *root, char *file, char *ext, int nsubdir,  char **subdir));
extern char *IDL_FilePath IDL_ARG_PROTO((char *pathbuf, char *file, char
        *ext, int nsubdir, char **subdir));
extern void *IDL_MemAlloc IDL_ARG_PROTO((unsigned int n, char *err_str,
        int action));
extern void IDL_MemFree IDL_ARG_PROTO((IDL_REGISTER void *m, char
        *err_str, int action));
extern void *IDL_MemAllocPerm IDL_ARG_PROTO((unsigned int n, char
        *err_str, int action));
extern void IDL_TimerSet IDL_ARG_PROTO((IDL_LONG length, IDL_TIMER_CB
        callback, int from_callback, IDL_TIMER_CONTEXT_PTR context));
...
extern void IDL_TimerSet IDL_ARG_PROTO((IDL_LONG length, IDL_TIMER_CB
        callback, int from_callback, IDL_TIMER_CONTEXT_PTR context));
extern void IDL_SignalSetInit IDL_ARG_PROTO((IDL_SignalSet_t *set, int
        signo));
extern void IDL_SignalSetAdd IDL_ARG_PROTO((IDL_SignalSet_t *set, int
        signo));
extern void IDL_SignalSetDel IDL_ARG_PROTO((IDL_SignalSet_t *set, int
        signo));
extern int IDL_SignalSetIsMember IDL_ARG_PROTO((IDL_SignalSet_t *set, int
        signo));
extern void IDL_SignalMaskGet IDL_ARG_PROTO((IDL_SignalSet_t *set));
extern void IDL_SignalMaskSet IDL_ARG_PROTO((IDL_SignalSet_t *set,
        IDL_SignalSet_t *oset));
extern void IDL_SignalMaskBlock IDL_ARG_PROTO((IDL_SignalSet_t *set,
        IDL_SignalSet_t *oset));
extern void IDL_SignalBlock IDL_ARG_PROTO((int signo, IDL_SignalSet_t
        *oset));
extern void IDL_SignalSuspend IDL_ARG_PROTO((IDL_SignalSet_t *set));
extern int IDL_SignalRegister IDL_ARG_PROTO((int signo,
        IDL_SignalHandler_t func, int msg_action));
extern int IDL_SignalUnregister IDL_ARG_PROTO((int signo,
        IDL_SignalHandler_t func, int msg_action));
...
extern void IDL_PolyfillSoftware IDL_ARG_PROTO((int *x, int *y, int n,
        IDL_POLYFILL_ATTR *s));
extern double IDL_GraphText IDL_ARG_PROTO((IDL_GR_PT *p, IDL_ATTR_STRUCT
        *ga, IDL_TEXT_STRUCT *a, char *text));
extern void IDL_StrDup IDL_ARG_PROTO((IDL_REGISTER IDL_STRING *str,
        IDL_REGISTER IDL_LONG n));
...
extern void IDL_WidgetIssueStubEvent IDL_ARG_PROTO((char *rec, IDL_LONG
        value));
extern void IDL_WidgetSetStubIds IDL_ARG_PROTO((char *rec, unsigned long
        t_id, unsigned long b_id));
extern void IDL_WidgetGetStubIds IDL_ARG_PROTO((char *rec, unsigned long
        *t_id, unsigned long *b_id));
...
extern int IDL_FindDefinition IDL_ARG_PROTO((char *routine, char
        **filename, int *line));
extern short IDL_SetRecallBufSize IDL_ARG_PROTO((short nSize));
extern char *IDL_MakeTempArray IDL_ARG_PROTO((int type, int n_dim,
        IDL_LONG dim[], int init, IDL_VPTR *var));
extern void IDL_EzCall IDL_ARG_PROTO((int argc, IDL_VPTR argv[],
        IDL_EZ_ARG arg_struct[]));
extern void IDL_EzCallCleanup IDL_ARG_PROTO((int argc, IDL_VPTR argv[],
        IDL_EZ_ARG arg_struct[]));
extern void IDL_MessageErrno IDL_ARG_PROTO((int code, ...));
extern void IDL_Message IDL_ARG_PROTO((int code, int action, ...));
extern void IDL_MessageVarError IDL_ARG_PROTO((int code, IDL_VPTR var, int
        action));
extern void IDL_MessageVMS IDL_ARG_PROTO((int code,...));
extern char *IDL_Rline IDL_ARG_PROTO((char *s, int n, int unit, FILE
...
extern int IDL_Win32Init IDL_ARG_PROTO((int iOpts, void *hinstExe, void
        *hwndExe, void *hAccel));
...
extern IDL_VPTR IDL_FindNamedVariable IDL_ARG_PROTO((char *name, int
        ienter));
...
extern char *IDL_MakeTempStruct IDL_ARG_PROTO((IDL_StructDefPtr sdef, int
        n_dim, IDL_LONG *dim, IDL_VPTR *var, int zero));
extern IDL_StructDefPtr IDL_MakeStruct IDL_ARG_PROTO((char *name,
        IDL_STRUCT_TAG_DEF *tags));
extern IDL_LONG IDL_StructTagInfoByName IDL_ARG_PROTO((IDL_StructDefPtr
        sdef, char *name, int msg_action, IDL_VPTR *var));
extern IDL_LONG IDL_StructTagInfoByIndex IDL_ARG_PROTO((IDL_StructDefPtr
        sdef, int index, int msg_action, IDL_VPTR *var));
extern void IDL_Print IDL_ARG_PROTO((int argc, IDL_VPTR *argv, char
        *argk));
extern void IDL_PrintF IDL_ARG_PROTO((int argc, IDL_VPTR *argv, char
        *argk));
extern void IDL_Win32MessageLoop  IDL_ARG_PROTO((int fFlush));
extern void IDL_VarCopy IDL_ARG_PROTO((IDL_REGISTER IDL_VPTR src,
        IDL_REGISTER IDL_VPTR dst));
extern void IDL_StoreScalar IDL_ARG_PROTO((IDL_VPTR dest, int type,
        IDL_ALLTYPES *value));
extern int IDL_KWGetParams IDL_ARG_PROTO((int argc, IDL_VPTR *argv, char
        *argk, IDL_KW_PAR *kw_list, IDL_VPTR *plain_args,  int imask));
...
extern void IDL_RasterDrawThick IDL_ARG_PROTO((IDL_GR_PT *p0, IDL_GR_PT
        *p1, IDL_ATTR_STRUCT *a, IDL_PRO_PTR routine, int dot_width));
extern void IDL_RasterPolyfill IDL_ARG_PROTO((int *x, int *y, int n,
        IDL_POLYFILL_ATTR *p, IDL_RASTER_DEF *r));
extern void IDL_RasterDraw IDL_ARG_PROTO((IDL_GR_PT *p0, IDL_GR_PT *p1,
        IDL_ATTR_STRUCT *a, IDL_RASTER_DEF *r));
extern void IDL_Dither IDL_ARG_PROTO((UCHAR *data, int ncols, int nrows,
        IDL_RASTER_DEF *r, int x0, int y0, IDL_TV_STRUCT *secondary));
extern void IDL_BitmapLandscape IDL_ARG_PROTO((IDL_RASTER_DEF *in,
        IDL_RASTER_DEF *out, int y0));
extern IDL_LONG IDL_LongScalar IDL_ARG_PROTO((IDL_REGISTER IDL_VPTR p));
extern double IDL_DoubleScalar IDL_ARG_PROTO((IDL_REGISTER IDL_VPTR p));
extern IDL_VPTR IDL_BasicTypeConversion IDL_ARG_PROTO((int argc, IDL_VPTR
...
extern IDL_VPTR IDL_CvtString IDL_ARG_PROTO((int argc, IDL_VPTR argv[],
        char *argk));
extern int IDL_GetKbrd IDL_ARG_PROTO((int should_wait));
extern void IDL_VarGetData IDL_ARG_PROTO((IDL_VPTR v, IDL_LONG *n, char
        **pd, int ensure_simple));
extern IDL_VPTR IDL_ImportArray IDL_ARG_PROTO((int n_dim, IDL_LONG dim[],
        int type, UCHAR *data, IDL_ARRAY_FREE_CB free_cb, 
        IDL_StructDefPtr s));
extern IDL_VPTR IDL_ImportNamedArray IDL_ARG_PROTO((char *name, int n_dim,
...
extern IDL_VPTR IDL_CvtBytscl IDL_ARG_PROTO((int argc, IDL_VPTR *argv,
        char *argk));
...
extern char *IDL_GetScratch IDL_ARG_PROTO((IDL_REGISTER IDL_VPTR *p,
        IDL_REGISTER IDL_LONG n_elts,  IDL_REGISTER IDL_LONG elt_size));
extern void IDL_RgbToHsv IDL_ARG_PROTO((UCHAR *r, UCHAR *g, UCHAR *b,
        float *h, float *s, float *v, int n));
extern void IDL_RgbToHls IDL_ARG_PROTO((UCHAR *r, UCHAR *g, UCHAR *b,
        float *h, float *l, float *s, int n));
extern int IDL_AddDevice IDL_ARG_PROTO(( IDL_DEVICE_DEF *dev,  int
        msg_action));
...
extern int IDL_AddSystemRoutine IDL_ARG_PROTO((IDL_SYSFUN_DEF *defs, int
        is_function, int cnt));
...
extern int IDL_Main IDL_ARG_PROTO((int init_options, int argc, char
        *argv[]));
...
extern void IDL_Runtime IDL_ARG_PROTO((int options, int *argc, char
        *argv[], char *file));
...
extern int IDL_FileEnsureStatus IDL_ARG_PROTO((int action, int unit, int
        flags));
extern int IDL_FileOpen IDL_ARG_PROTO((int argc, IDL_VPTR argv[], char
        *argk, int access_mode, int extra_flags, int longjmp_safe));
extern void IDL_FileClose IDL_ARG_PROTO((int argc, IDL_VPTR argv[], char
        *argk));
...
extern void IDL_FileStat IDL_ARG_PROTO((int unit, IDL_FILE_STAT
        *stat_blk));
