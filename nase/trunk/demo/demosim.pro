...
;
; PURPOSE:
;  This IDL program is inteded to demonstrate the usage of some important
;  routines of the N.A.S.E. package.<BR> 
;  In particular, these routines are:<BR>
;  - <A>InitPara_1</A>, <A>InitLayer</A>, <A>InputLayer</A>,
;  <A>ProceedLayer</A>, <A>FreeLayer</A> to process layers of model
;  neurons, <BR>
;
;  - <A>InitDW</A>, <A>DelayWeigh</A>, <A>FreeDW</A>, to initialize
;  and update connections between neurons of certain layers,<BR>
;
;  - <A>InitRecall</A>, <A>TotalRecall</A>, <A>LearnHebbLP</A>,
;  <A>FreeRecall</A>, to modify connection strengths according to
;  learning rules, and<BR>
;
;  - <A>ShowWeights</A>, <A>Plotcilloscope</A>, <A>Trainspottingscope</A>, 
;  to display the current state of the simulated network. <BR>
;  <BR>
;  The example network consists of a layer made up of 10x10 neurons
;  (called <*>L1</*> in the source code) and an inhibitory interneuron
;  (called <*>L2</*>).<BR>
;  Neurons of the layer receive a pattern showing two squares as their
;  feeding input (<*>I_L1_F</*>). Further inputs are linking signals
;  from all cells inside their own layer (<*>CON_L1_L1</*>) and
;  inhibitory input from the interneuron (<*>CON_L2_L1</*>). The
;  strengths of the linking connections are adapted during the
;  simulation according to a Hebbian learning rule.<BR>
;  The inhibitory interneuron receives its feeding input from all
;  neurons in layer <*>L1</*> (<*>CON_L1_L2</*>).<BR>
;  During simulation, action potentials generated by the <*>L1</*>
;  neurons and by the interneuron are displayed. Furthermore, the
;  membrane potential, the inhibitory input and the dynamic threshold
;  of one of the neurons in <*>L1</*> is shown. In a separate window,
;  the strength of the connections that are learned is displayed.<BR>
; <BR>
;  See also comments in the source code for further explanations.
;          
; CATEGORY:
;  Demonstration
;  NASE
