...
;                                  [SAMPLPERIOD=samplperiod] , [PLOT=plot] , [NOSAMPLE = nosample])
;
; 
...
;                                       SHIFTHUELL : 1d-Array von der Position des Maximums der Einhuellenden, s.o.
;                                       SHIFTMAX   : 1d-Array von der Position des Maximums der Kreuzkorrelation , s.o.
;                                       SHIFTMIN   : 1d-Array von der Position des Minimums der Kreuzkorrelation , s.o.
...
function slidcorr , xdata , ydata , taxis ,FBAND=fband, WSIZE=wsize , STEPSIZE=stepsize, NOPERIOD=noperiod,$
   ...
   ;;stop
   XSIZE = 600 &  YSIZE=320
   IF set(plot) AND PLOTFLAG EQ 0 THEN BEGIN
   ...
   DEFAULT,fband,[0,fdata(N_ELEMENTS(fdata)-1)]
   ...
   FOR K=0,ergsparrdim(1)-1 DO BEGIN
      ...
      chuell = ABS(chuell)
      cspectmp = reform(ergsparr(k,*))
      energie = total(abs(cspectmp([findex_R,findex_L])))/total(abs(cspectmp))
