...
;  For reasons of simplicity and clarity, <*>EnvelopeCorrelation</*> does not pass on to the user every option of the
;  routines it calls; only those parameters can be manipulated for which this usually makes sense. Actually, this routine
;  is suited for real data analysis purposes only when the number of (signal) channels is very small. <I>Never ever</I>
;  use it when you want to compute all pair-wise AEC values among more than, say, 10 channels; this would be much too
;  time-consuming, because for each channel the envelope signals and their slices would be computed several times.
;  In this case, write your own program, taking over the main steps from this source code.<BR>
...
;
; CALLING SEQUENCE:
;* ecxy = EnvelopeCorrelation(x, y, fX, fY, fS  [, BANDWIDTH=...] [, FLANKWIDTH=...] [, SSIZE=...] [, SSHIFT=...]
;
; INPUTS:
...
;
; OUTPUTS:
;  ecxy::  A float array containing the AEC values. The first dimension represents the <*>x</*> pass-band centre
...
;* EnvCxy  = EnvelopeCorrelation(X, Y, 5*FIndGen(9)+30, 5*FIndGen(9)+30, 500.0)
;* EnvCxyM = FZT( Total(FZT(EnvCxy),4)/100, 1 )
;* Print, StdDev(EnvCxy), StdDev(EnvCxyM)
;
...
FUNCTION  EnvelopeCorrelation,   X, Y, fX_, fY_, fS_,  $
                                 bandwidth = bandwidth_, flankwidth = flankwidth_, ssize = ssize_, sshift = sshift_
...
;   On_Error, 2
   ...
   EnvX = Make_Array(dimension = [NfX,DimsX], type = 4, /nozero)   ; array for the X envelopes (frequency bands in 1st dimension)
   EnvY = Make_Array(dimension = [NfY,DimsX], type = 4, /nozero)   ; array for the Y envelopes (frequency bands in 1st dimension)
   ; Determination of the envelopes:
   FOR  ifX = 0, NfX-1  DO  $
     EnvX[ifX,*,*,*,*,*,*,*] = Envelope(Float(X), fS, flow = fX[ifX]-BandWidth/2, fhigh = fX[ifX]+BandWidth/2,  $
                                                      wlow = FlankWidth, whigh = FlankWidth, /hertz)
   FOR  ifY = 0, NfY-1  DO  $
     EnvY[ifY,*,*,*,*,*,*,*] = Envelope(Float(Y), fS, flow = fY[ifY]-BandWidth/2, fhigh = fY[ifY]+BandWidth/2,  $
                                                      wlow = FlankWidth, whigh = FlankWidth, /hertz)
   ...
   EnvX = Transpose(EnvX, Permutation)
   EnvY = Transpose(EnvY, Permutation)
   ; The envelopes signals are "time-sliced" and made mean-free:
   EnvXSlices = RemoveMean(Slices(EnvX, ssize = SSize, sshift = SShift, sampleperiod = 1/fS, /tfirst), 1)
   EnvYSlices = RemoveMean(Slices(EnvY, ssize = SSize, sshift = SShift, sampleperiod = 1/fS, /tfirst), 1)
   ; Now the dimension coding the different frequency bands is needed in the first position again:
   IF  NfX EQ 1  THEN  EnvXSlices = Reform(EnvXSlices, [1, Size(EnvXSlices, /dim)], /overwrite)  $
                 ELSE  EnvXSlices = Transpose(EnvXSlices, [SizeX[0]+1 , IndGen(SizeX[0]+1)])
   IF  NfY EQ 1  THEN  EnvYSlices = Reform(EnvYSlices, [1, Size(EnvYSlices, /dim)], /overwrite)  $
