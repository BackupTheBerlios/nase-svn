...
n = nx * ny
u = reform(exp(-sqrt(xp^2 + yp^2)), n)	;Exp() term, Make it 1D
F = a(0) + a(1) * u
...
Function Exp2d_fit, z, a, x, y, XCENTER=xcenter, YCENTER=ycenter, NEGATIVE = neg, TILT=tilt,$
                CONVERGED=converged
;+
; NAME:
;	EXP2d_FIT
...
;	a(1) = A1 = scale factor.
;	a(2) = a = width of exponential in X direction.
;	a(3) = b = width of exponential in Y direction.
...
;	NEGATIVE = if set, implies that the exponential to be fitted
...
;       XCENTER/
;       YCENTER = an initial guess where the center of the exponential might be
...
;	converge.
;
; EXAMPLE:  This example creates a 2D exponential, adds random noise
;	and then applies EXP2DFIT:
...
;	z = a(0) + a(1) * exp(-u)		;to exponential
...
ix = i mod nx
iy = i / nx
x0 = x(ix)         ;enthaelt wohl die max-position
y0 = y(iy)         ;enthaelt wohl die max-position

; First guess, without XY term...
IF NOT Set(XCENTER) THEN xcenter = x0 ;use presumable center location
IF NOT Set(YCENTER) THEN ycenter = y0

;Guess at params by taking slices using only the positive branch
;pass inverted version to 'ucomfit' because we have exp decay
;Use Minimum of the respective slice as guess for offset
;Global peak value Q as guess for factor 
;And start with base=1.0 (which means no exponential dependency at all)
...
ax_exp = ucomfit(-x(ix:*)+ix, z(ix:*,iy), [factorx,basex,offsetx], yfit=savefitx, /exponential) 
ay_exp = ucomfit(-y(iy:*)+iy, z(ix,iy:*), [factory,basey,offsety], yfit=savefity, /exponential)
...
;Translate the coeffs of the preliminary exp slice fit to the
;order expected by EXP2__FUNCT (passed by ucurvefit) 


a = [	(offsetx+offsety)/2.0, $        ;mean value as guess
        sqrt(factorx*factory), $	;geometric mean as guess
        expdivisory, expdivisorx, $     ;anisotropic exponential divisors
        xcenter, ycenter]	        ;presumable center location
...
result = ucurvefit([nx, ny, x, y], reform(z, n, /OVERWRITE), $
		...
		function_name = "EXP2__FUNCT", /NODERIVATIVE, CONVERGED=converged)
