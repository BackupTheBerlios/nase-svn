    ...
    AND n_elements(fjac) GT 0 AND n_elements(ipvt) GT 0 then begin
      ...
      if n GT 0 AND sz(0) GT 1 AND sz(1) GE n AND sz(2) GE n $
        AND n_elements(ipvt) GE n then begin
          ...
          if n EQ 1 then $
            cv_ = mpfit_covar(reform([fjac(0,0)],1,1), ipvt(0)) $ ;changed variable name cv->cv_
          else $
            cv_ = mpfit_covar(fjac(0:n-1,0:n-1), ipvt(0:n-1))
          cv_ = reform(cv_, n, n, /overwrite)
          nn = n_elements(xall)
          
          ;; Fill in actual covariance matrix, accounting for fixed
          ;; parameters.
          covar = replicate(cv_(0)*0, nn, nn)
          for i = 0L, n-1 do begin
              covar(ifree, ifree(i)) = cv_(*,i)
          end
